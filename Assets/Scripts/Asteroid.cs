...
public class Asteroid : MonoBehaviourPun {
    ...
    public float curveReduction = 0.5f;
    ...
    private float spawnTimer = 0, thrustDelay = 0, applyForce = 0;
    public bool IsDoneSpawning {
        get {return spawnTimer > activateAfterSpawning;}
    }

    private GameObject asteroidSpinner;
    ...
    void Start() {
        ...
        asteroidSpinner = new GameObject("Asteroid Spinner");
        asteroidSpinner.transform.position = Vector3.zero;
        asteroidSpinner.transform.localScale = Vector3.one;
        asteroidSpinner.transform.SetParent(transform.parent, true);
        transform.SetParent(asteroidSpinner.transform, true);
        rotateSpeed = swirlSpeed;
    }
    ...
    void FixedUpdate() {
        if(asteroidSpinner != null && PhotonNetwork.IsMasterClient) {
            asteroidSpinner.transform.Rotate(0, 0, rotateSpeed);
            rotateSpeed = Mathf.Lerp(rotateSpeed, 0, Time.deltaTime * curveReduction);

            if(Util.Approximately(rotateSpeed, 0, 1f)) {
                photonView.RPC("RemoveSpin", RpcTarget.All);
                return;
            }
        }
    }
    ...
    [PunRPC]
    protected void RemoveSpin() {
        if(asteroidSpinner == null) return;
        transform.SetParent(asteroidSpinner.transform.parent, true);
        Destroy(asteroidSpinner);
    }
